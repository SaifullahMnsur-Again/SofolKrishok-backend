...
def _load_model():
    """Load the soil classification model and its classes."""
    global _soil_model, _soil_model_cache_key, _soil_classes
    ...
    if artifact:
        ...
        if _soil_model is not None and _soil_model_cache_key == cache_key:
            return _soil_model, _soil_classes
        ...
        if not loaded_classes:
            raise ValueError(f"No classes found for active soil model '{artifact.display_name}' in the database.")
    ...
    raise ValueError("No active soil classification model found in the database.")
...
def classify_soil(image_file) -> dict:
    ...
    predicted_type = classes.get(predicted_idx, f"Unknown Type {predicted_idx}")

    # All predictions sorted by confidence
    all_preds = {}
    for idx, score in enumerate(scores):
        soil_name = classes.get(idx, f"Type_{idx}")
